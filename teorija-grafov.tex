...
\begin{document}
	...
	\begin{definition}[Petersenov graf]
		...
		$u$-ji so povezani v cikel. Enakole"zni $u$-ji in $v$-ji so povezani. $v$-ji so povezani s preskoki po $k$.
		\\
	...
	\begin{definition}[Kro"zni grafi]
		Vzemimo poljubno mno"zico $S \subset \Z_n$, za katero velja $0 \notin S \land (s \in S \implies (n-s) \in S)$. Tedaj je kro"zni graf $\operatorname{Cir}(n, S)$ tak graf, katerega vozli"s"ca tvori $\Z_n$, povezave pa $\left\lbrace uv, v-u \in S\right\rbrace$.
	\end{definition}
	$K_n = \operatorname{Cir}(n, [n-1])$ \\ 
	$C_n = \operatorname{Cir}(n, \left\lbrace1, n-1\right\rbrace)$
...
\section{Poti, cikli in dvodelnost}
	...
	\begin{theorem}
		...
		\begin{proof} 
			...
			($\Leftarrow$) predpostavimo lahko, da je $G$ povezan. ("Ce ni povezan, je dvodelen natanko tedaj, ko so vse komponente dvodelne). Vzemimo vozli"s"ce $v_0$. Definirajmo $A=\left\lbrace x \in V(G): d(v_0, x) \text{ je sodo}\right\rbrace$ in $B=\left\lbrace x \in V(G): d(v_0, x) \text{ je liho}\right\rbrace$ (tukaj nam pomaga, da smo predpostavili povezanost, saj nam morebitna neskon"cna razdalja ne dela preglavic). O"citno je $A \cap B = \emptyset \land A \cup B = V(G)$. Poka"zimo, da je $(A,B)$ dvodelna razdelitev (znotraj $A$ in znotraj $B$ ni povezav): naj bosta $u,w \in A$. Lo"cimo 3 primere:
			\begin{enumerate}
	...
	"Ce povezavo nadomestimo s potjo dol"zine 2, smo povezavo subdividirali. Ozna"cimo $G^+(E)$ \\ 
	...
	\begin{definition}[Glajenje vozli"s"c] Neformalno: obratna operacija subdividiranja. \\
		Formalno: 
		$$ V(G- \lbrace w \rbrace )) = V(G) \backslash \left\lbrace w\right\rbrace $$ 
		$$ E(G- \lbrace w \rbrace ) = (E(G) \backslash \left\lbrace uw, vw \right\rbrace) \cup \left\lbrace uv \right\rbrace $$
	\end{definition}
	...
	\end{definition}
	\begin{definition}[Prerezna povezava]
		$e$ je prerezna (aka. most), "ce je $\Omega(G-e) > \Omega(G)$.
	\end{definition}
	\begin{definition}[k-povezan graf]
		"Ce (ima vsaj $k+1$ vozli"s"c in) ne premore nobenega prereza mo"ci $< k$ (prerez tu pomeni mno"zico odstranjenih vozli"s"c). Z drugimi besedami, $k$-povezanemu grafu moramo odstraniti najmanj $k$ vozli"s"c, da pove"camo "stevilo njegovih komponent.
	\end{definition}
	\begin{definition}[Povezanost $G$]
		Najve"cji $k$ da je $G$ $k$-povezan. Ozna"cimo $\kappa(G)$
	\end{definition}
	...
	\begin{definition}[Povezavna povezanost $G$]
		Je najmanj"se "stevilo povezav, ki jih moramo odstraniti, da pove"camo "stevilo komponent $G$. Ozna"cimo $\kappa '(G)$
	\end{definition}
	
	\[ \kappa (G) \leq \kappa '(G) \leq \delta(G) \]
	% delta je minimalna stopnja vozlisca v $G$
	% kappa je povezanost, kappa' pa povezavna povezanost $G$
	...
	\begin{definition}[Drevo s korenom]
		Drevo, v katerem izberemo vozli"s"ce in ga imenujemo ``koren''. Ponavadi ri"semo koren na vrhu ali dnu drevesa.
	\end{definition}
	...
	\begin{lemma}
		...
		\end{proof}
	\end{lemma}
	\begin{lemma}\label{odstranjevanjeCiklov}
	...
	\begin{claim}
		...
		\begin{proof}
			($\Leftarrow$) O"citno. \\
			($\Rightarrow$) "Ce je $G$ drevo je sam sebi vpeto drevo. Sicer vsebuje cikel. Na povezavi cikla uporabimo lemo \ref{odstranjevanjeCiklov}. Uni"cujemo cikle dokler ne dose"zemo drevesa. To drevo je vpeto.
		\end{proof}
	\end{claim}
	
	\begin{definition}[$\tau(G)$]
		"Stevilo vpetih dreves $G$.
	\end{definition}
	$\tau(\text{drevo}) = 1$ \\
	$\tau(C_n) = n$ \\
	...
	Obstajajo (med drugim) bijektivni dokazi Cayleyevega izreka (vpeta drevesa spravimo v bijektivno korespondenco z nekimi objekti, ki jih znamo pre"steti). En tak na"cin je Pr\"uferjeva koda.
	...
	\begin{claim}
		"Ce je $G$ graf, ki ima sama soda vozli"s"ca, potem $G$ premore dekompozicijo v cikle.	
		\begin{proof}
	...
	\begin{claim}
		...
		\begin{proof}
			"Ce je $G$ povezan graf s ciklom in $f$ lice vlo"zitve, potem je $l(f) \geq g(G)$. Po prej"snji trditvi dobimo:
	...
	$\chi(K_n) = n$ \\ $\chi(C_n) = 2\text{, "ce je }n\text{ sod, sicer }3$ \\ $\chi(P(5,2)) = 3$
